...
public class MapChunk
{
    ...
    private void RenderQueueBakeMeshes()
    {
        ...
        Vector3[] mtvertices;//Current mesh vertices
        int indOffset;//Ofset of indices for vertex array
        ...
        {
            if (!e.Current.Key.name.StartsWith("Tile"))//Filter out non-tile objects
            {
            ...
            {
                mt = ma[i];
                mtvertices = mt.vertices;
                indOffset = vertices.Count;//Save index offset

                for (j = 0; j < mt.vertexCount; j++)//Go through all vertices
                    vertices.Add(mtvertices[j]);//Add vertex

                tind = mt.GetIndices(0);//Get indices of current mesh
                for (j = 0; j < tind.Length; j++)//Go through them
                    indices.Add(tind[j] + indOffset);//Offset them and add to index list
            }
    ...
    private void GenerateVisibilityOverlayMesh()
    {
        ...
        int[] tind;//List of indices of current mesh
        Vector3[] mtvertices;//Currnet mesh vertices
        int indOffset;//Ofset of indices for vertex array
        ...
        while (e.MoveNext())//Go through all material-mesh combinations
        {
            ...
            mt = ma[0];//Combination contains only one mesh after queue baking
            mtvertices = mt.vertices;
            indOffset = vertices.Count;

            for (i = 0; i < mt.vertexCount; i++)//Loop through all vertices
            {
                vertices.Add(tv3 = mtvertices[i]);//Add vertex to list of vertices
            ...
            for (i = 0; i < tind.Length; i++)//Loop through all indices
                indices.Add(tind[i] + indOffset);//Offset them and add to index list
